...
DROP TRIGGER IF EXISTS trg_comentarios_cambios_agenda;

DELIMITER $$

CREATE TRIGGER trg_comentarios_cambios_agenda
AFTER UPDATE ON agenda
FOR EACH ROW
BEGIN
    DECLARE v_usuario VARCHAR(50);

    SET v_usuario = COALESCE(NULLIF(NEW.creado_por, ''), '0');

    IF NOT (OLD.id_consultorio <=> NEW.id_consultorio) THEN
        INSERT INTO comentarios_agenda (comentario, cod_agendaFK)
        VALUES (LEFT(CONCAT('@{0}: El usuario @{', v_usuario, '} ha modificado id_consultorio de ', COALESCE(CAST(OLD.id_consultorio AS CHAR), 'NULL'), ' a ', COALESCE(CAST(NEW.id_consultorio AS CHAR), 'NULL')), 255), NEW.id_agenda);
    END IF;

    IF NOT (OLD.id_paciente <=> NEW.id_paciente) THEN
        INSERT INTO comentarios_agenda (comentario, cod_agendaFK)
        VALUES (LEFT(CONCAT('@{0}: El usuario @{', v_usuario, '} ha modificado id_paciente de ', COALESCE(CAST(OLD.id_paciente AS CHAR), 'NULL'), ' a ', COALESCE(CAST(NEW.id_paciente AS CHAR), 'NULL')), 255), NEW.id_agenda);
    END IF;

    IF NOT (OLD.id_profesional <=> NEW.id_profesional) THEN
        INSERT INTO comentarios_agenda (comentario, cod_agendaFK)
        VALUES (LEFT(CONCAT('@{0}: El usuario @{', v_usuario, '} ha modificado id_profesional de ', COALESCE(CAST(OLD.id_profesional AS CHAR), 'NULL'), ' a ', COALESCE(CAST(NEW.id_profesional AS CHAR), 'NULL')), 255), NEW.id_agenda);
    END IF;

    IF NOT (OLD.fecha <=> NEW.fecha) THEN
        INSERT INTO comentarios_agenda (comentario, cod_agendaFK)
        VALUES (LEFT(CONCAT('@{0}: El usuario @{', v_usuario, '} ha modificado fecha de ', COALESCE(CAST(OLD.fecha AS CHAR), 'NULL'), ' a ', COALESCE(CAST(NEW.fecha AS CHAR), 'NULL')), 255), NEW.id_agenda);
    END IF;

    IF NOT (OLD.hora_inicio <=> NEW.hora_inicio) THEN
        INSERT INTO comentarios_agenda (comentario, cod_agendaFK)
        VALUES (LEFT(CONCAT('@{0}: El usuario @{', v_usuario, '} ha modificado hora_inicio de ', COALESCE(CAST(OLD.hora_inicio AS CHAR), 'NULL'), ' a ', COALESCE(CAST(NEW.hora_inicio AS CHAR), 'NULL')), 255), NEW.id_agenda);
    END IF;

    IF NOT (OLD.hora_fin <=> NEW.hora_fin) THEN
        INSERT INTO comentarios_agenda (comentario, cod_agendaFK)
        VALUES (LEFT(CONCAT('@{0}: El usuario @{', v_usuario, '} ha modificado hora_fin de ', COALESCE(CAST(OLD.hora_fin AS CHAR), 'NULL'), ' a ', COALESCE(CAST(NEW.hora_fin AS CHAR), 'NULL')), 255), NEW.id_agenda);
    END IF;

    IF NOT (OLD.motivo <=> NEW.motivo) THEN
        INSERT INTO comentarios_agenda (comentario, cod_agendaFK)
        VALUES (LEFT(CONCAT('@{0}: El usuario @{', v_usuario, '} ha modificado motivo de ', COALESCE(OLD.motivo, 'NULL'), ' a ', COALESCE(NEW.motivo, 'NULL')), 255), NEW.id_agenda);
    END IF;

    IF NOT (OLD.estado <=> NEW.estado) THEN
        INSERT INTO comentarios_agenda (comentario, cod_agendaFK)
        VALUES (LEFT(CONCAT('@{0}: El usuario @{', v_usuario, '} ha modificado estado de ', COALESCE(OLD.estado, 'NULL'), ' a ', COALESCE(NEW.estado, 'NULL')), 255), NEW.id_agenda);
    END IF;

    IF NOT (OLD.color <=> NEW.color) THEN
        INSERT INTO comentarios_agenda (comentario, cod_agendaFK)
        VALUES (LEFT(CONCAT('@{0}: El usuario @{', v_usuario, '} ha modificado color de ', COALESCE(OLD.color, 'NULL'), ' a ', COALESCE(NEW.color, 'NULL')), 255), NEW.id_agenda);
    END IF;

    IF NOT (OLD.cod_ventaFK <=> NEW.cod_ventaFK) THEN
        INSERT INTO comentarios_agenda (comentario, cod_agendaFK)
        VALUES (LEFT(CONCAT('@{0}: El usuario @{', v_usuario, '} ha modificado cod_ventaFK de ', COALESCE(CAST(OLD.cod_ventaFK AS CHAR), 'NULL'), ' a ', COALESCE(CAST(NEW.cod_ventaFK AS CHAR), 'NULL')), 255), NEW.id_agenda);
    END IF;

    IF NOT (OLD.cod_presupuestoFK <=> NEW.cod_presupuestoFK) THEN
        INSERT INTO comentarios_agenda (comentario, cod_agendaFK)
        VALUES (LEFT(CONCAT('@{0}: El usuario @{', v_usuario, '} ha modificado cod_presupuestoFK de ', COALESCE(CAST(OLD.cod_presupuestoFK AS CHAR), 'NULL'), ' a ', COALESCE(CAST(NEW.cod_presupuestoFK AS CHAR), 'NULL')), 255), NEW.id_agenda);
    END IF;
END$$
