...
-- Agrega el Diagrama de gant como acceso rapido sin tocar tareas ni datos del Gantt.
SET @diagrama_gant_catalog_existe := (
  SELECT COUNT(*)
  FROM dashboard_access_catalog
  WHERE access_key = 'diagrama_gant'
);

UPDATE dashboard_access_catalog
SET default_quick_order = default_quick_order + 1
WHERE @diagrama_gant_catalog_existe = 0
  AND is_default_quick_access = 1
  AND default_quick_order >= 15;
...
VALUES
  ('diagrama_gant', 'Diagrama de gant', 'administrativo', 'Administrativo', 'diagrama-gant', '/GoodVentaAsisCap/php_system/Grant.php', NULL, 1, 1, 15)
...
INNER JOIN dashboard_access_catalog catalogo
  ON catalogo.access_key = 'diagrama_gant'
