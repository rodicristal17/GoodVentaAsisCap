...
var agendaTratamientosPlanMap = {};
var agendaPlanMadreActual = null;
var agendaPlanesMadreActuales = [];
var agendaPlanMadreSeleccionadoId = "";
var agendaPlanMadreAcordeonAbiertoId = "";
...
var agendaEventoDetalleActual = null;
...
function conservarPlanificacionConsultoriosAgenda(consultoriosNuevos, consultoriosAnteriores){
    var anteriores = {};
    (consultoriosAnteriores || []).forEach(function(consultorio){
        anteriores[String(consultorio.id)] = consultorio;
    });
    return (consultoriosNuevos || []).map(function(consultorio){
        var anterior = anteriores[String(consultorio.id)];
        if(!Object.prototype.hasOwnProperty.call(consultorio, 'profesionales_planificados') && anterior){
            consultorio.profesionales_planificados = anterior.profesionales_planificados || [];
        }
        return consultorio;
    });
}
...
function aplicarRespuestaCargaAgenda(datosRespuesta, completo){
    ...
    agendaConsultoriosData.consultorios = conservarPlanificacionConsultoriosAgenda(
        datosRespuesta["consultorios"] || [],
...
function fusionarCargaIncrementalConsultoriosAgenda(datosRespuesta, idsActualizados, completo){
    var ids = {};
    var consultoriosRespuesta = conservarPlanificacionConsultoriosAgenda(
        datosRespuesta["consultorios"] || [],
        agendaConsultoriosData.consultorios
    );
...
function obtenerColorProfesionalAgenda(codProfesional){
    var colores = [
        "#07899a", "#7357d9", "#ee6b19", "#3aa61b", "#e9487d",
        "#2563eb", "#9c4dcc", "#c47b00", "#00876b", "#d34040"
    ];
    var texto = String(codProfesional || "0");
    var hash = 0;
    var i;

    if(window.TelarProfessionalIdentity && typeof window.TelarProfessionalIdentity.color === "function"){
        return window.TelarProfessionalIdentity.color(codProfesional);
    }
...
function obtenerInicialesProfesionalAgenda(nombre){
    var palabras;
    if(window.TelarProfessionalIdentity && typeof window.TelarProfessionalIdentity.initials === "function"){
        return window.TelarProfessionalIdentity.initials(nombre);
    }
    palabras = String(nombre || "?").trim().split(/\s+/);
    return (palabras.length > 1
        ? palabras[0].charAt(0) + palabras[palabras.length - 1].charAt(0)
...
function renderAvatarProfesionalAgenda(profesional){
    var avatar = String(profesional.avatar || "");
    var iniciales = escaparHtmlAgenda(obtenerInicialesProfesionalAgenda(profesional.nombre || ""));
    var html = "<span class='agenda-plan-avatar' aria-hidden='true'>";
    if(avatar !== ""){
        html += "<img src='" + escaparHtmlAgenda(avatar) + "' alt='' "
            + "onerror=\"this.hidden=true;this.nextElementSibling.hidden=false\">"
            + "<span class='agenda-plan-avatar__fallback' hidden>" + iniciales + "</span>";
    }else{
        html += "<span class='agenda-plan-avatar__fallback'>" + iniciales + "</span>";
    }
    return html + "</span>";
}

function renderProfesionalesPlanificadosAgenda(consultorio){
    var profesionales = obtenerProfesionalesPlanificadosConsultorioAgenda(consultorio);
    var html = "";
    ...
    for(i = 0; i < profesionales.length; i++){
        ...
        html += "<span class='agenda-plan-profesional agenda-plan-profesional--"
            + escaparHtmlAgenda(profesional.estado || "planificada") + "' "
            + "style='--agenda-plan-color:" + escaparHtmlAgenda(color) + "' "
            + "title='" + escaparHtmlAgenda(titulo) + "'>"
            + renderAvatarProfesionalAgenda(profesional)
            + "<span class='agenda-plan-profesional__texto'><strong>"
            + escaparHtmlAgenda(profesional.nombre || "Profesional") + "</strong><small>"
            + detalle + "</small></span>"
            + "<span class='agenda-plan-profesional__nodo' aria-hidden='true'></span>"
            + "</span>";
    }
    return html + "</div>";
}

function profesionalPlanificadoActivoEnIntervaloAgenda(profesional, inicio, fin){
    var inicioMin = horaAMinutos(inicio);
    var finMin = horaAMinutos(fin);
    ...
    profesionalInicio = horaAMinutos(profesional.hora_entrada);
    profesionalFin = horaAMinutos(profesional.hora_salida);
    return profesionalInicio < finMin && profesionalFin > inicioMin;
}

function renderHilosPlanificacionSlotAgenda(idConsultorio, horaInicio){
    var consultorio = obtenerConsultorioAgendaPorId(idConsultorio);
    var profesionales = obtenerProfesionalesPlanificadosConsultorioAgenda(consultorio);
    var finMin = horaAMinutos(horaInicio) + 30;
    var horaFin = completarHora(Math.floor(finMin / 60)) + ":" + completarHora(finMin % 60);
    var html = "";
    var activos = [];
    var i, profesional, color, clase;

    for(i = 0; i < profesionales.length; i++){
        if(profesionalPlanificadoActivoEnIntervaloAgenda(profesionales[i], horaInicio, horaFin)){
            activos.push(profesionales[i]);
        }
    }
    for(i = 0; i < activos.length; i++){
        profesional = activos[i];
        color = obtenerColorProfesionalAgenda(profesional.cod_profesional);
        clase = profesional.estado === "propuesta"
            ? " agenda-plan-hilo--propuesta"
            : (profesional.estado === "pendiente_horario"
                ? " agenda-plan-hilo--pendiente" : " agenda-plan-hilo--confirmado");
        html += "<span class='agenda-plan-hilo" + clase + "' aria-hidden='true' "
            + "style='--agenda-plan-color:" + escaparHtmlAgenda(color)
            + ";--agenda-plan-offset:" + (5 + (i * 5)) + "px'></span>";
    }
    return html;
}

function obtenerProfesionalVinculadoEventoAgenda(evento){
    var consultorio = obtenerConsultorioAgendaPorId(evento.consultorio);
    var profesionales = obtenerProfesionalesPlanificadosConsultorioAgenda(consultorio);
    var inicioEvento = horaAMinutos(evento.inicio);
    var finEvento = horaAMinutos(evento.fin);
    var i, profesional, inicioProfesional, finProfesional;

    for(i = 0; i < profesionales.length; i++){
        profesional = profesionales[i];
        if(profesional.vincula_turnos !== true
            || !profesional.hora_entrada || !profesional.hora_salida){
            continue;
        }
        inicioProfesional = horaAMinutos(profesional.hora_entrada);
        finProfesional = horaAMinutos(profesional.hora_salida);
        if(inicioEvento >= inicioProfesional && finEvento <= finProfesional){
            return profesional;
        }
    }
    return null;
...
function renderNodoProfesionalEventoAgenda(evento){
    var profesional = obtenerProfesionalVinculadoEventoAgenda(evento);
    var color;
    if(!profesional){
        return "";
    }
    color = obtenerColorProfesionalAgenda(profesional.cod_profesional);
    return "<span class='agenda-evento-plan-nodo' aria-hidden='true' "
        + "style='--agenda-plan-color:" + escaparHtmlAgenda(color) + "'></span>";
}
...
function pintarEventosAgenda(fecha, estado, consultoriosSeleccionados){
    ...
    for(i = 0; i < slots.length; i++){
        ...
        htmlFinal = datosAlmuerzo.htmlOverlay
            + renderHilosPlanificacionSlotAgenda(consultorioId, horaSlot)
            + htmlEventos;
...
function cargarEventosResumenInsumosSemanaAgenda(idConsultorio, fecha, periodo){
    ...
    if(codLocal == '' && !verTodosConsultorios && selectLocalAgenda && selectLocalAgenda.options.length <= 1 && typeof cod_localFKUSer !== "undefined"){
...
function isValidAppointmentStateForOccupancy(estado){
    ...
    return ['CANCELADO', 'ANULADO', 'ANULADA', 'ELIMINADO', 'ELIMINADA', 'INACTIVO', 'AUSENTE', 'AUSENCIA', 'NO ASISTIO', 'NOASISTIO', 'NO_ASISTIO'].indexOf(estado) < 0;
}
...
function renderEventoAgenda(e, eventosMismoConsultorio){
    ...
    var eventoNoArrastrable = e.estado === "FERIADO" || e.estado === "AUSENTE";
    var zIndex = (e.estado === "FERIADO") ? "0" : "10"; // Los feriados van al fondo
    var cursorEstilo = eventoNoArrastrable ? "cursor: default;" : "";
...
function renderTratamientosEventoAgenda(textoTratamientos){
    ...
    var html = "";
    var i, parte, texto, coincidencia, nombre, porcentaje, riesgoMatch, riesgoHtml, nivelRiesgo, textoRiesgo, claseFilaTratamiento;
    var riesgoRegex = /<span\b[^>]*class=(['"])[^'"]*riesgo-financiero-badge[^'"]*riesgo-nivel-(\d)[^'"]*\1[^>]*>(.*?)<\/span>/i;
    ...
    for(i = 0; i < partes.length; i++){
        parte = partes[i] || "";
        riesgoMatch = parte.match(riesgoRegex);
        riesgoHtml = "";
        if(riesgoMatch){
            nivelRiesgo = Math.max(1, Math.min(5, parseInt(riesgoMatch[2], 10) || 1));
            textoRiesgo = (riesgoMatch[3] || ("N" + nivelRiesgo)).replace(/<[^>]*>/g, '').trim();
            riesgoHtml = "<span class='riesgo-financiero-badge agenda-evento-riesgo riesgo-nivel-" + nivelRiesgo + "' title='Riesgo financiero'>" + escaparHtmlAgenda(textoRiesgo) + "</span>";
            parte = parte.replace(riesgoRegex, "");
        }
        texto = parte.replace(/<[^>]*>/g, '').trim();
        if(texto === ''){ continue; }
        ...
        claseFilaTratamiento = riesgoHtml ? " agenda-evento-tratamiento--con-riesgo" : "";

        html += "<span class='agenda-evento-tratamiento" + claseFilaTratamiento + "' style='--avance-tratamiento:" + porcentaje + "%'>"
...
function obtenerTextoTratamientosTituloAgenda(textoTratamientos){
    return String(textoTratamientos || '')
        .replace(/<span\b[^>]*class=(['"])[^'"]*riesgo-financiero-badge[^'"]*\1[^>]*>.*?<\/span>/gi, '')
        .replace(/<br\s*\/?>/gi, ', ')
        .replace(/<[^>]*>/g, '')
        .replace(/\s+/g, ' ')
        .trim();
...
function actualizarResumenFiltrosAgenda(){
    ...
    document.getElementById('agendaFiltrosActivos').innerHTML = '<button class="btn-accion-principal" onclick="verCerrarModalFeriados(true)">Feriados</button>'
        + '<button class="btn-filtro agenda-btn-planificacion" onclick="abrirPlanificacionAgendaContextual()" title="Abrir la planificaci&oacute;n vinculada a esta fecha y sucursal"><i class="fa-solid fa-share-nodes" aria-hidden="true"></i> Asignar consultorios</button>'
        + "<span class='chip-filtro'>Fecha: " + fecha + "</span>"
        + "<span class='chip-filtro'>Consultorios: " + textoConsultorio + "</span>";
}

function abrirPlanificacionAgendaContextual(){
    var inputFecha = document.getElementById('inptFechaAgenda');
    var filtroLocal = document.getElementById('inptLocalAgendaFiltro');
    var consultoriosSeleccionados = obtenerConsultoriosSeleccionadosAgenda();
    var codLocal = filtroLocal ? String(filtroLocal.value || '') : '';
    var i, consultorio;

    if(codLocal === ''){
        for(i = 0; i < consultoriosSeleccionados.length; i++){
            consultorio = obtenerConsultorioAgendaPorId(consultoriosSeleccionados[i]);
            if(consultorio && consultorio.cod_localFk){
                codLocal = String(consultorio.cod_localFk);
                break;
            }
        }
    }
    if(typeof window.abrirPlanificacionEspecialistasDesdeAgenda !== 'function'){
        window.alert('La pantalla de Planificaci\u00f3n no est\u00e1 disponible. Actualice la p\u00e1gina e intente nuevamente.');
        return;
    }
    window.abrirPlanificacionEspecialistasDesdeAgenda(
...
function guardarCitaAgenda(){
    ...
    actualizarVentaSeleccionadaDesdeTratamientosAgenda();
    var codVenta = agendaVentaSeleccionada || "";
...
function limpiarFormularioNuevaCita(){
    ...
    if(document.getElementById('inptIdPacienteAgenda')){
        document.getElementById('inptIdPacienteAgenda').value = '';
    }
    ...
    if(document.getElementById('inptDoctorNuevaCita')){
        document.getElementById('inptDoctorNuevaCita').innerHTML = '';
    }
    if(document.getElementById('divPlanMadreNuevaCitaAgenda')){
        document.getElementById('divPlanMadreNuevaCitaAgenda').innerHTML = "<span class='agenda-ayuda-inline'>Seleccione un paciente.</span>";
    }
    if(document.getElementById('divTratamientosNuevaCitaAgenda')){
        document.getElementById('divTratamientosNuevaCitaAgenda').innerHTML = "<p class='agenda-ayuda-inline'>Seleccione un paciente para ver su plan madre.</p>";
    }
    ...
    agendaTratamientosVentaMap = {};
    agendaTratamientosPlanMap = {};
    agendaPlanMadreActual = null;
    agendaPlanesMadreActuales = [];
    agendaPlanMadreSeleccionadoId = "";
    agendaPlanMadreAcordeonAbiertoId = "";
    idAbmAgenda= "";
}
		...
		success: function (responseText) {
			...
			try {
				...
				if (Respuesta == true) {
                    ...
                    var notas = motivoBase + (datos["3"] || '');
                    contenedorComentarios.innerHTML = notas || "<div class='detalle-notas-empty'>Todavia no se registraron notas.</div>";
				}else{
    				ver_vetana_informativa("LO SENTIMOS HA OCURRIDO UN ERROR ")
				}
			} catch (error) {
...
function verificarComentarioAgendamiento() {
    ...
    var botonEnviarComentario = document.querySelector('.detalle-send-action');
    ...
    if (botonEnviarComentario) {
        botonEnviarComentario.disabled = true;
	...
	$.ajax({
        ...
        complete: function () {
            ...
            if (botonEnviarComentario) {
                botonEnviarComentario.disabled = false;
...
function verDetalleAgenda(id){
    ...
    cargarPlanMadrePacienteAgenda(
        evento.cod_cliente || evento.id_paciente || '',
        null,
        evento.id || '',
        'detAgendaPlanMadreEstado'
    );
    renderPrevisionInsumosAgenda(evento);
    var tratamientosTituloAgenda = obtenerTextoTratamientosTituloAgenda(evento.nombres_tratamiento || '');
    document.getElementById('detAgendaTitulo').innerHTML = "Detalle del turno";
    document.getElementById('detAgendaPresupuesto').innerHTML = escaparHtmlAgenda(evento.nombre_doctor || 'Sin doctor asignado');
    document.getElementById('detAgendaFecha').innerHTML = escaparHtmlAgenda(formatearFechaDetalleAgenda(evento.fecha || ''));
    if(document.getElementById('detAgendaHorarioTexto')){
        document.getElementById('detAgendaHorarioTexto').innerHTML = escaparHtmlAgenda((evento.inicio || '') + (evento.fin ? " a " + evento.fin : ""));
    }
    if(document.getElementById('detAgendaDuracion')){
        document.getElementById('detAgendaDuracion').innerHTML = escaparHtmlAgenda(calcularDuracionDetalleAgenda(evento.inicio || '', evento.fin || '') || 'Sin duracion');
    }
    if(document.getElementById('detAgendaConsultorio')){
        document.getElementById('detAgendaConsultorio').innerHTML = escaparHtmlAgenda(datosConsultorioDetalle.nombre || 'Sin consultorio');
    }
    if(document.getElementById('detAgendaSucursal')){
        document.getElementById('detAgendaSucursal').innerHTML = escaparHtmlAgenda(datosConsultorioDetalle.sucursal || 'Sin sucursal');
    }
    if(document.getElementById('detAgendaFechaReprogramar')){
        document.getElementById('detAgendaFechaReprogramar').value = evento.fecha || '';
    }
    ...
    actualizarEstadoVisualDetalleAgenda(evento);
    actualizarAccionesDetalleAgenda(evento);
    ...
    document.getElementById('inptNroDocCliente').value= String(evento.ci_cliente || '').replace(/\B(?=(\d{3})+(?!\d))/g, ".") || '';
    document.getElementById('inptNroRucCliente').value= evento.rut_cliente || '';
    document.getElementById('inptNroTelefCliente').value= evento.telefono || '';
    document.getElementById('inptNrowhatsappCliente').value= evento.whapp || '';
    document.getElementById('inptDireccionCliente').value= evento.direccion || '';
    ...
    if(document.getElementById('detAgendaDeudaAlerta')){
        document.getElementById('detAgendaDeudaAlerta').innerHTML = 'Cargando situacion financiera...';
        document.getElementById('detAgendaDeudaAlerta').style.display = '';
    }
...
function obtenerEventoDetalleAgendaActual(){
    return agendaEventoDetalleActual || null;
}

function obtenerConsultorioDetalleAgenda(evento){
    var consultorio = null;
    for(var i = 0; i < agendaConsultoriosData.consultorios.length; i++){
        if(String(agendaConsultoriosData.consultorios[i].id) === String(evento.consultorio)){
            consultorio = agendaConsultoriosData.consultorios[i];
            break;
        }
    }
    return {
        nombre: consultorio ? (consultorio.nombre || "") : "",
        sucursal: consultorio ? obtenerSucursalConsultorioAgenda(consultorio) : ""
    };
}

function formatearFechaDetalleAgenda(fecha){
...
function calcularDuracionDetalleAgenda(inicio, fin){
    var minutos = horaAMinutos(fin) - horaAMinutos(inicio);
...
function estadoPrincipalDetalleAgenda(estado){
    estado = String(estado || "").toUpperCase();
    if(estado === "CONFIRMADOCONDEUDA"){
        return "CONFIRMADO";
    }
    if(estado === "PRIMERACONSULTA"){
        return "AGENDADO";
    }
    if(estado === "AUSENCIA" || estado === "NO_ASISTIO" || estado === "NOASISTIO" || estado === "NO ASISTIO"){
        return "AUSENTE";
    }
    return estado || "AGENDADO";
}

function esEstadoFinalDetalleAgenda(estado){
    var estadoPrincipal = estadoPrincipalDetalleAgenda(estado);
    return estadoPrincipal === "ATENDIDO" || estadoPrincipal === "CANCELADO" || estadoPrincipal === "AUSENTE";
}

function etiquetaEstadoDetalleAgenda(estado){
    var estadoPrincipal = estadoPrincipalDetalleAgenda(estado);
    if(estadoPrincipal === "AGENDADO"){ return "Agendado"; }
    if(estadoPrincipal === "CONFIRMADO"){ return "Confirmado"; }
    if(estadoPrincipal === "ATENDIDO"){ return "Atendido"; }
    if(estadoPrincipal === "CANCELADO"){ return "Cancelado"; }
    if(estadoPrincipal === "AUSENTE"){ return "Ausente"; }
    if(estadoPrincipal === "ENESPERA"){ return "En espera"; }
    return estadoPrincipal;
}

function tipoCitaDetalleAgenda(evento){
    var estado = String(evento.estado || "").toUpperCase();
    if(estado === "PRIMERACONSULTA"){
        return "Primera consulta";
    }
    if(evento.tratamientos_ids && evento.tratamientos_ids.length > 0){
        return "Tratamiento";
    }
    if(evento.nombres_tratamiento){
        return "Tratamiento";
    }
    return "Control";
}
...
function renderTratamientosDetalleAgenda(textoTratamientos){
    ...
    for(var i = 0; i < partes.length; i++){
        var parte = partes[i] || "";
        var texto = parte.replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim();
        if(texto === ""){
            continue;
        }
...
function actualizarEstadoVisualDetalleAgenda(evento){
    var estado = estadoPrincipalDetalleAgenda(evento.estado);
    var contEstado = document.getElementById("detAgendaEstado");
    var tipo = document.getElementById("detAgendaTipoCita");
    if(contEstado){
        contEstado.innerHTML = "<span class='badge-estado-detalle badge-" + escaparHtmlAgenda(estado) + "'>" + escaparHtmlAgenda(etiquetaEstadoDetalleAgenda(evento.estado)) + "</span>";
    }
    if(tipo){
        tipo.textContent = tipoCitaDetalleAgenda(evento);
    }
}

function resetDeudaDetalleAgenda(){
    var alerta = document.getElementById("detAgendaDeudaAlerta");
    ...
    if(detalle){
        ...
        detalle.innerHTML = "";
        detalle.setAttribute("data-tiene-deuda", "0");
        detalle.setAttribute("data-cuotas-atrasadas", "0");
    }
}

function actualizarDeudaDetalleAgenda(cuotasAtrasadas, detalleHtml){
    cuotasAtrasadas = parseInt(cuotasAtrasadas, 10) || 0;
    var alerta = document.getElementById("detAgendaDeudaAlerta");
    var detalle = document.getElementById("detAgendaDeudasPendientes");
    if(!detalle){ return; }
    ...
    if(alerta){
        if(cuotasAtrasadas > 0){
            alerta.innerHTML = "<span>Tiene " + cuotasAtrasadas + " cuota" + (cuotasAtrasadas == 1 ? "" : "s") + " atrasada" + (cuotasAtrasadas == 1 ? "" : "s") + "</span><button type='button' class='detalle-link-action' onclick='toggleDetalleDeudaAgenda()'>Ver detalle</button>";
    ...
    actualizarAccionesDetalleAgenda(agendaEventoDetalleActual);
}
...
function actualizarAccionesDetalleAgenda(evento){
    if(!evento){ return; }
    var estado = estadoPrincipalDetalleAgenda(evento.estado);
    var deuda = document.getElementById("detAgendaDeudasPendientes");
    var tieneDeuda = deuda && deuda.getAttribute("data-tiene-deuda") == "1";
    var btnConfirmar = document.getElementById("btnConfirmAgendamiento");
    var btnConfirmarDeuda = document.getElementById("btnConfirmDeudaAgendamiento");
    var btnAtendido = document.getElementById("btnAtendidoAgendamiento");
    ...
    var btnAsignarTratamientos = document.getElementById("detAgendaBtnAsignarTratamientos");
    ...
    var esFinal = esEstadoFinalDetalleAgenda(evento.estado);

    [btnConfirmar, btnConfirmarDeuda, btnAtendido, btnFinal].forEach(function(btn){
    ...
    if(btnAsignarTratamientos){
        btnAsignarTratamientos.textContent = evento.sin_tratamiento ? "Asignar tratamientos" : "Cambiar / agregar tratamientos";
    }
    if(btnPrimeraConsulta){
        btnPrimeraConsulta.style.display = (estadoOriginal === "PRIMERACONSULTA" || esFinal) ? "none" : "";
    }
    if(btnCancelar){
        btnCancelar.style.display = esFinal ? "none" : "";
    }

    if(estado === "AGENDADO" || estado === "ENESPERA"){
        if(tieneDeuda && btnConfirmarDeuda){
            btnConfirmarDeuda.style.display = "";
    ...
    if(estado === "CONFIRMADO"){
        if(btnAtendido){ btnAtendido.style.display = ""; }
        return;
    }

    if(btnFinal){
        btnFinal.textContent = estado === "CANCELADO" ? "Cita cancelada" : (estado === "ATENDIDO" ? "Atencion completada" : (estado === "AUSENTE" ? "Ausencia registrada" : etiquetaEstadoDetalleAgenda(evento.estado)));
...
function setAccionesDetalleAgendaProcesando(procesando){
    ...
    for(var i = 0; i < botones.length; i++){
        ...
        botones[i].disabled = agendaDetalleAccionEnProceso;
        botones[i].style.opacity = agendaDetalleAccionEnProceso ? "0.65" : "";
    }
}

function toggleMasAccionesDetalleAgenda(){
    var menu = document.getElementById("detAgendaMasAccionesMenu");
    if(!menu){ return; }
    menu.style.display = menu.style.display == "none" ? "" : "none";
}

function cerrarMasAccionesDetalleAgenda(){
    var menu = document.getElementById("detAgendaMasAccionesMenu");
...
function cerrarReprogramacionDetalleAgenda(){
    var panel = document.getElementById("detAgendaReprogramarPanel");
    if(panel){ panel.style.display = "none"; }
    if(agendaEventoDetalleActual){
        if(document.getElementById("detAgendaFechaReprogramar")){
            document.getElementById("detAgendaFechaReprogramar").value = agendaEventoDetalleActual.fecha || "";
        }
        document.getElementById("detAgendaHorarioInicio").value = agendaEventoDetalleActual.inicio || "";
        document.getElementById("detAgendaHorarioFin").value = agendaEventoDetalleActual.fin || "";
    }
}

function confirmarReprogramacionDetalleAgenda(){
    cerrarMasAccionesDetalleAgenda();
    actualizarHorarioAgendaDesdeModal();
}

function confirmarCancelarCitaDetalleAgenda(){
    var evento = agendaEventoDetalleActual || {};
    if(esEstadoFinalDetalleAgenda(evento.estado)){
        cerrarMasAccionesDetalleAgenda();
        return;
    }
    var mensaje = "Se cancelara la cita de " + (evento.paciente || "este paciente")
        + " del " + formatearFechaDetalleAgenda(evento.fecha || "")
        + ", de " + (evento.inicio || "") + " a " + (evento.fin || "") + ".\n\nEsta accion modificara el estado del turno.";
    cerrarMasAccionesDetalleAgenda();
    if(confirm(mensaje)){
        cambiarEstadoAgendaDesdeModal("CANCELADO");
    }
}

function manejarTeclaComentarioAgendamiento(event){
    if(event.key === "Enter" && !event.shiftKey){
        event.preventDefault();
        verificarComentarioAgendamiento();
    }
}

function renderEstadoCargaDetalleAgenda(){
    resetDeudaDetalleAgenda();
    var historial = document.getElementById("table_historial_paciente_agenda_detalle");
    var notas = document.getElementById("detAgendaMotivo");
    if(historial){ historial.innerHTML = "<div class='detalle-notas-empty'>Cargando ultimas citas...</div>"; }
    if(notas){ notas.innerHTML = "<div class='detalle-notas-empty'>Cargando notas...</div>"; }
}

function renderHistorialDetalleAgenda(tabla, html){
    agendaDetalleHistorialCompletoVisible = false;
    var boton = document.getElementById("detAgendaVerHistorialCompleto");
    var temporal = document.createElement("div");
    temporal.innerHTML = html || "";
    var filas = temporal.querySelectorAll("tr");
    var items = [];
    var idActual = document.getElementById("detAgendaId") ? document.getElementById("detAgendaId").textContent.trim() : "";

    for(var i = 0; i < filas.length; i++){
        var fila = filas[i];
        var idFila = fila.getAttribute("data-agenda-id") || "";
        if(idActual !== "" && idFila !== "" && String(idFila) === String(idActual)){
            continue;
        }
        var celdas = fila.children;
        if(celdas.length < 3){ continue; }
        var fecha = celdas[0].textContent.trim();
        var horario = celdas[1].textContent.trim();
        var estadoHtml = celdas[2].innerHTML;
        var onclick = fila.getAttribute("onclick") || "";
        items.push({
            fecha: fecha,
            horario: horario,
            estado: estadoHtml,
    ...
    if(items.length === 0){
        tabla.innerHTML = "<div class='detalle-notas-empty'>Sin citas anteriores registradas.</div>";
    ...
    for(var j = 0; j < items.length; j++){
        var oculto = j >= 4 ? " style='display:none;'" : "";
        salida += "<div class='detalle-historial-item' data-historial-extra='" + (j >= 4 ? "1" : "0") + "'" + oculto + (items[j].onclick ? " onclick='" + items[j].onclick.replace(/'/g, "&#039;") + "'" : "") + ">"
            + "<div><strong>" + escaparHtmlAgenda(items[j].fecha) + "</strong><span>" + escaparHtmlAgenda(items[j].horario) + "</span></div>"
            + "<div>" + items[j].estado + "</div>"
            + "</div>";
    }
    tabla.innerHTML = salida;
    if(boton){
        boton.style.display = items.length > 4 ? "" : "none";
        boton.textContent = "Ver historial completo";
    }
}

function toggleHistorialCompletoDetalleAgenda(){
    agendaDetalleHistorialCompletoVisible = !agendaDetalleHistorialCompletoVisible;
    var extras = document.querySelectorAll("#table_historial_paciente_agenda_detalle [data-historial-extra='1']");
    for(var i = 0; i < extras.length; i++){
        extras[i].style.display = agendaDetalleHistorialCompletoVisible ? "" : "none";
    }
    var boton = document.getElementById("detAgendaVerHistorialCompleto");
    if(boton){
        boton.textContent = agendaDetalleHistorialCompletoVisible ? "Ver menos" : "Ver historial completo";
    }
}
...
function renderPrevisionInsumosAgenda(evento){
    ...
    html += "<details class='detalle-insumos-resumen'>";
    html += "<summary>" + insumos.length + " insumo" + (insumos.length == 1 ? "" : "s") + " calculado" + (insumos.length == 1 ? "" : "s") + "</summary>";
    html += "<div class='agenda-insumos-lista'>";
...
function actualizarVentaSeleccionadaDesdeTratamientosAgenda(){
    agendaVentaSeleccionada = "";
    for(var i = 0; i < agendaTratamientosSeleccionados.length; i++){
        var detalle = String(agendaTratamientosSeleccionados[i] || "");
        if(detalle !== "" && agendaTratamientosVentaMap[detalle]){
            agendaVentaSeleccionada = String(agendaTratamientosVentaMap[detalle]);
            return;
        }
    }
}

function renderResumenPlanMadreAgenda(resp, idResumen){
    ...
    if(planes.length > 0){
        ...
        for(var i = 0; i < planes.length; i++){
            var planItem = planes[i].plan || {};
            var requiere = Number(planItem.requiere_aprobacion || 0) === 1;
            if(requiere){ pendientes++; }
            chips += "<span class='agenda-plan-madre-chip " + (requiere ? "agenda-plan-madre-chip--pendiente" : "") + "'>"
                + "<b>#" + escaparHtmlAgenda(planItem.numero || (i + 1)) + "</b>"
                + escaparHtmlAgenda((planItem.label || "Plan madre").replace(/^Plan madre #[0-9]+ - /, ""))
                + "</span>";
        }
        resumen.innerHTML = "<div class='agenda-plan-madre-resumen " + (pendientes > 0 ? "agenda-plan-madre-resumen--pendiente" : "agenda-plan-madre-resumen--ok") + "'>"
            + "<span>Planes madre del paciente</span>"
            + "<strong>" + planes.length + " plan" + (planes.length == 1 ? " madre disponible" : "es madre disponibles") + "</strong>"
            + "<em>Seleccione tratamientos de un solo plan madre por cita.</em>"
            + (pendientes > 0 ? "<small>" + pendientes + " plan" + (pendientes == 1 ? "" : "es") + " con falta de aprobacion del doctor de cabecera.</small>" : "")
            + "<div class='agenda-plan-madre-chips'>" + chips + "</div>"
            + "</div>";
        return;
    }
    var plan = resp && resp.plan ? resp.plan : null;
    if(plan){
        var pendiente = String(plan.pendiente_validacion || "0") == "1";
        var borrador = String(plan.estado || "").toLowerCase() == "borrador";
        var tituloPlan = borrador ? "Plan madre en borrador" : "Plan madre vigente";
        resumen.innerHTML = "<div class='agenda-plan-madre-resumen " + ((pendiente || borrador) ? "agenda-plan-madre-resumen--pendiente" : "agenda-plan-madre-resumen--ok") + "'>"
            + "<span>" + tituloPlan + "</span>"
            + "<strong>" + escaparHtmlAgenda(plan.label || "Plan madre") + "</strong>"
            + "<em>" + escaparHtmlAgenda(plan.estado_texto || "Vigente") + "</em>"
            + (pendiente ? "<small>Puede agendarse. Queda pendiente de revision del jefe clinico.</small>" : "")
            + (borrador ? "<small>Se respeta la ruta cargada para evitar duplicar un plan existente.</small>" : "")
            + "</div>";
        return;
    }
    var total = resp && resp.tratamientos ? resp.tratamientos.length : 0;
    resumen.innerHTML = "<div class='agenda-plan-madre-resumen agenda-plan-madre-resumen--sin-plan'>"
        + "<span>Sin plan madre vigente</span>"
        + "<strong>Generar plan madre sugerido</strong>"
        + "<em>" + total + " tratamiento" + (total == 1 ? "" : "s") + " disponible" + (total == 1 ? "" : "s") + "</em>"
        + "<small>El sistema lo creara como vigente y pendiente de validacion clinica.</small>"
        + "</div>";
}

function motivoBloqueoTratamientoAgenda(tratamiento, progreso){
    var estado = String(tratamiento.estado_clase || tratamiento.estado || "").toLowerCase();
    var texto = String(tratamiento.estado_texto || "").toLowerCase();
    if(estado === "completado" || estado === "realizado" || texto.indexOf("realizado") >= 0 || progreso >= 100){
        return "Ya realizado. No disponible para nueva cita.";
    }
    if(estado === "cancelado" || estado === "anulado" || texto.indexOf("anulado") >= 0 || texto.indexOf("cancelado") >= 0){
        return "Anulado. No disponible para agendar.";
    }
    return "No disponible para seleccionar en esta cita.";
}

function renderTratamientoSeleccionAgenda(tratamiento, opciones){
    opciones = opciones || {};
    var detalleId = String(tratamiento.cod_detalle || "");
    var avance = typeof tratamiento.avance !== "undefined" ? tratamiento.avance : tratamiento.progreso_porcentaje;
    var progreso = Math.max(0, Math.min(100, parseInt(avance || 0, 10)));
    var nivelRiesgo = Math.max(1, Math.min(5, parseInt(tratamiento.nivel_riesgo_financiero || 1, 10)));
    var textoRiesgo = tratamiento.nivel_riesgo_texto || ("N" + nivelRiesgo);
    var claseRiesgo = String(tratamiento.nivel_riesgo_clase || ("riesgo-nivel-" + nivelRiesgo)).replace(/[^A-Za-z0-9_ -]/g, "");
    var tieneSeleccionable = Object.prototype.hasOwnProperty.call(tratamiento, "seleccionable");
    var seleccionable = tieneSeleccionable ? Number(tratamiento.seleccionable) === 1 : true;
    var seleccionado = Number(tratamiento.seleccionado) === 1;
    var planId = String(opciones.planId || tratamiento.plan_id || "");
    var planLabel = String(opciones.planLabel || "");
    var orden = tratamiento.orden || opciones.orden || "";
    var ventaId = tratamiento.venta_id || tratamiento.cod_ventaFK || "";
    var motivoBloqueo = seleccionable ? "" : motivoBloqueoTratamientoAgenda(tratamiento, progreso);
    var atributosPlan = planId !== ""
        ? " data-plan-id='" + escaparHtmlAgenda(planId) + "' data-plan-label='" + escaparHtmlAgenda(planLabel) + "'"
        : "";
    var clases = "tratamiento-agenda-card tratamiento-agenda-check" + (!seleccionable ? " tratamiento-agenda-check--bloqueado" : "");
    return "<label class='" + clases + "'" + (motivoBloqueo ? " title='" + escaparHtmlAgenda(motivoBloqueo) + "'" : "") + ">"
        + "<input type='checkbox' value='" + escaparHtmlAgenda(detalleId) + "'" + atributosPlan + " " + (seleccionado ? "checked" : "") + (!seleccionable ? " disabled aria-disabled='true'" : "") + " onchange='toggleTratamientoAgenda(this)'>"
        + (orden !== "" ? "<span class='tratamiento-agenda-orden'>N" + escaparHtmlAgenda(orden) + "</span>" : "")
        + "<span class='tratamiento-agenda-nombre'>" + escaparHtmlAgenda(tratamiento.nombre_producto || "") + "</span>"
        + "<span class='riesgo-financiero-badge agenda-riesgo-badge " + claseRiesgo + "' title='Riesgo financiero'>" + escaparHtmlAgenda(textoRiesgo) + "</span>"
        + "<span class='tratamiento-agenda-estado tratamiento-agenda-estado--" + escaparHtmlAgenda(tratamiento.estado_clase || "pendiente") + "'>" + escaparHtmlAgenda(tratamiento.estado_texto || "") + "</span>"
        + "<span class='tratamiento-agenda-progreso'>" + progreso + "%</span>"
        + (ventaId !== "" ? "<small>Venta #" + escaparHtmlAgenda(ventaId) + "</small>" : "")
        + (motivoBloqueo ? "<span class='tratamiento-agenda-bloqueo'>" + escaparHtmlAgenda(motivoBloqueo) + "</span>" : "")
        + "</label>";
}

function renderTratamientosPlanMadreAgenda(resp, idContenedor, idResumen, idAgenda){
    var cont = idContenedor ? document.getElementById(idContenedor) : null;
    renderResumenPlanMadreAgenda(resp, idResumen);
    agendaPlanMadreActual = resp && resp.plan ? resp.plan : null;
    agendaPlanesMadreActuales = resp && resp.planes ? resp.planes : [];
    agendaTratamientosVentaMap = {};
    agendaTratamientosPlanMap = {};
    agendaTratamientosSeleccionados = [];
    agendaVentaSeleccionada = "";
    ...
    var tratamientos = resp && resp.tratamientos ? resp.tratamientos : [];
    for(var i = 0; i < tratamientos.length; i++){
        var detalle = String(tratamientos[i].cod_detalle || "");
        var planTratamiento = String(tratamientos[i].plan_id || "");
        if(detalle !== ""){
            agendaTratamientosVentaMap[detalle] = String(tratamientos[i].venta_id || "");
            agendaTratamientosPlanMap[detalle] = planTratamiento;
        }
        if(Number(tratamientos[i].seleccionado) === 1){
            agendaTratamientosSeleccionados.push(detalle);
            if(agendaPlanMadreSeleccionadoId === "" && planTratamiento !== ""){
                agendaPlanMadreSeleccionadoId = planTratamiento;
            }
        }
    }
    actualizarVentaSeleccionadaDesdeTratamientosAgenda();
    ...
    if(resp && resp.modo === "sin_plan"){
        var pacienteId = (resp.paciente && resp.paciente.paciente_id) ? resp.paciente.paciente_id : "";
        var htmlSinPlan = "<div class='agenda-plan-madre-alerta'>"
            + "<strong>Este paciente no tiene plan madre vigente.</strong>"
            + "<span>Para agendar tratamientos, primero genere el plan madre sugerido con el orden recomendado por el sistema.</span>"
            + "<button type='button' class='btn-accion-principal agenda-plan-madre-alerta__btn' onclick='crearPlanMadreSugeridoAgenda(\"" + escaparHtmlAgenda(pacienteId) + "\",\"" + escaparHtmlAgenda(idContenedor || "") + "\",\"" + escaparHtmlAgenda(idAgenda || "") + "\",\"" + escaparHtmlAgenda(idResumen || "") + "\")'>Usar plan madre sugerido</button>"
            + "</div>";
        if(tratamientos.length > 0){
            htmlSinPlan += "<div class='agenda-plan-sugerido-preview'>";
            for(var s = 0; s < tratamientos.length; s++){
                htmlSinPlan += "<div class='tratamiento-agenda-card tratamiento-agenda-card--preview'>"
                    + "<span class='tratamiento-agenda-orden'>N" + escaparHtmlAgenda(tratamientos[s].orden || (s + 1)) + "</span>"
                    + "<span class='tratamiento-agenda-nombre'>" + escaparHtmlAgenda(tratamientos[s].nombre_producto || "") + "</span>"
                    + "<span class='tratamiento-agenda-estado'>" + escaparHtmlAgenda(tratamientos[s].estado_texto || "") + "</span>"
                    + "<small>Venta #" + escaparHtmlAgenda(tratamientos[s].venta_id || "") + "</small>"
                    + "</div>";
            }
            htmlSinPlan += "</div>";
        }else{
            htmlSinPlan += "<p class='agenda-ayuda-inline'>No hay tratamientos disponibles para generar el plan madre.</p>";
        }
        cont.innerHTML = htmlSinPlan;
        actualizarPrevisionNuevaCitaAgenda();
        return;
    }
    ...
    var html = "";
    if(planes.length > 0){
        var planAbiertoId = agendaPlanMadreSeleccionadoId;
        var existePlanAbierto = false;
        for(var ap = 0; ap < planes.length; ap++){
            var planDisponible = planes[ap].plan || {};
            var planDisponibleId = String(planDisponible.id || "");
            if(planAbiertoId !== "" && planDisponibleId === planAbiertoId){
                existePlanAbierto = true;
            }
        }
        if(planAbiertoId === "" || !existePlanAbierto){
            planAbiertoId = String((planes[0].plan || {}).id || "");
        }
        agendaPlanMadreAcordeonAbiertoId = planAbiertoId;
        html += "<div class='agenda-planes-madre-lista'>";
        for(var p = 0; p < planes.length; p++){
            var plan = planes[p].plan || {};
            var tratamientosPlan = planes[p].tratamientos || [];
            var requiereAprobacion = Number(plan.requiere_aprobacion || 0) === 1;
            var planId = String(plan.id || "");
            var planAbierto = planId !== "" && planId === agendaPlanMadreAcordeonAbiertoId;
            var planSeleccionado = planId !== "" && planId === agendaPlanMadreSeleccionadoId;
            html += "<section class='agenda-plan-madre-bloque " + (requiereAprobacion ? "agenda-plan-madre-bloque--pendiente " : "") + (planAbierto ? "agenda-plan-madre-bloque--abierto " : "") + (planSeleccionado ? "agenda-plan-madre-bloque--seleccionado" : "") + "' data-plan-id='" + escaparHtmlAgenda(planId) + "'>"
                + "<button type='button' class='agenda-plan-madre-bloque__header' aria-expanded='" + (planAbierto ? "true" : "false") + "' onclick='togglePlanMadreBloqueAgenda(\"" + escaparHtmlAgenda(planId) + "\")'>"
                + "<span class='agenda-plan-madre-bloque__numero'>#" + escaparHtmlAgenda(plan.numero || (p + 1)) + "</span>"
                + "<div>"
                + "<strong>" + escaparHtmlAgenda(plan.label || "Plan madre") + "</strong>"
                + "<small>" + escaparHtmlAgenda(plan.estado_texto || "Vigente") + "</small>"
                + (requiereAprobacion ? "<em>" + escaparHtmlAgenda(plan.aprobacion_texto || "Falta aprobacion del doctor de cabecera") + "</em>" : "")
                + "</div>"
                + "<b>" + tratamientosPlan.length + " trat.</b>"
                + "<span class='agenda-plan-madre-bloque__chevron'>" + (planAbierto ? "Ocultar" : "Ver") + "</span>"
                + "</button>";
            if(tratamientosPlan.length > 0){
                html += "<div class='agenda-plan-madre-bloque__tratamientos'>";
                for(var j = 0; j < tratamientosPlan.length; j++){
                    html += renderTratamientoSeleccionAgenda(tratamientosPlan[j], {
                        planId: planId,
                        planLabel: plan.label || "Plan madre",
                        orden: j + 1
                    });
                }
                html += "</div>";
            }else{
                html += "<p class='agenda-ayuda-inline'>Este plan madre no tiene tratamientos activos.</p>";
            }
            html += "</section>";
        }
        html += "</div>";
    }else{
        for(var j = 0; j < tratamientos.length; j++){
            html += renderTratamientoSeleccionAgenda(tratamientos[j], { orden: j + 1 });
        }
    }
    cont.innerHTML = html || "<p class='agenda-ayuda-inline'>El plan madre no tiene tratamientos activos.</p>";
    actualizarEstadoSeleccionPlanMadreAgenda();
    actualizarPrevisionNuevaCitaAgenda();
}

function cargarPlanMadrePacienteAgenda(idPaciente, idContenedor, idAgenda, idResumen){
    var cont = idContenedor ? document.getElementById(idContenedor) : null;
    agendaPlanMadreActual = null;
    agendaPlanesMadreActuales = [];
    agendaTratamientosVentaMap = {};
    ...
    agendaPlanMadreAcordeonAbiertoId = "";
    if(cont){
        cont.innerHTML = idPaciente ? paginacargando : "<p class='agenda-ayuda-inline'>Seleccione un paciente para ver su plan madre.</p>";
    }
    if(idResumen && document.getElementById(idResumen)){
        document.getElementById(idResumen).innerHTML = idPaciente ? "<span class='agenda-ayuda-inline'>Cargando plan madre...</span>" : "<span class='agenda-ayuda-inline'>Seleccione un paciente.</span>";
    }
    actualizarPrevisionNuevaCitaAgenda();
    if(!idPaciente){ return; }
    obtener_datos_user();
    $.ajax({
        data: {"useru": userid, "passu": passuser, "navegador": navegador, "paciente": idPaciente, "id_agenda": idAgenda || "", "funt": "obtenerPlanMadreAgenda"},
        url: "/GoodVentaAsisCap/php_system/abmCalendar.php",
        type: "post",
        success: function(responseText){
            try{
                var resp = typeof responseText === "string" ? $.parseJSON(responseText) : responseText;
                if(!respuestaJqueryAjax(resp["1"])){
                    if(cont){ cont.innerHTML = "<p class='agenda-alerta-inline'>" + escaparHtmlAgenda(resp.mensaje || "No se pudo cargar el plan madre.") + "</p>"; }
                    return;
                }
                renderTratamientosPlanMadreAgenda(resp, idContenedor, idResumen, idAgenda);
            }catch(error){
                console.error(error, responseText);
                if(cont){ cont.innerHTML = "<p class='agenda-alerta-inline'>No se pudo cargar el plan madre.</p>"; }
            }
        },
        error: function(jqXHR, textstatus, errorThrowm){
            console.error(jqXHR, textstatus, errorThrowm);
...
function crearPlanMadreSugeridoAgenda(idPaciente, idContenedor, idAgenda, idResumen){
    if(!idPaciente){ return; }
    if(!confirm("Se generara el plan madre vigente con el orden sugerido por el sistema. Quedara pendiente de validacion clinica. Desea continuar?")){
        return;
    }
    var cont = idContenedor ? document.getElementById(idContenedor) : null;
    if(cont){ cont.innerHTML = paginacargando; }
    obtener_datos_user();
    $.ajax({
        data: {"useru": userid, "passu": passuser, "navegador": navegador, "paciente": idPaciente, "funt": "crearPlanMadreSugeridoAgenda"},
        url: "/GoodVentaAsisCap/php_system/abmCalendar.php",
        type: "post",
        success: function(responseText){
            try{
                var resp = typeof responseText === "string" ? $.parseJSON(responseText) : responseText;
                if(respuestaJqueryAjax(resp["1"])){
                    ver_vetana_informativa(resp.mensaje || "Plan madre generado.");
                    cargarPlanMadrePacienteAgenda(idPaciente, idContenedor, idAgenda, idResumen);
                }else{
                    ver_vetana_informativa(resp.mensaje || "No se pudo generar el plan madre.", "", "error");
                    cargarPlanMadrePacienteAgenda(idPaciente, idContenedor, idAgenda, idResumen);
                }
            }catch(error){
                console.error(error, responseText);
                if(cont){ cont.innerHTML = "<p class='agenda-alerta-inline'>No se pudo generar el plan madre.</p>"; }
            }
        },
        error: function(jqXHR, textstatus, errorThrowm){
            console.error(jqXHR, textstatus, errorThrowm);
        ...
        success: function(responseText){
            try{
                ...
                for(var i = 0; i < tratamientos.length; i++){
                    ...
                    html += renderTratamientoSeleccionAgenda(tratamientos[i], { orden: i + 1 });
                }
...
function mostrarAvisoPlanMadreDiferenteAgenda(){
    var mensaje = "Esta cita solo puede incluir tratamientos de un plan madre. Para realizar otro tratamiento de otro plan madre, genere otra cita o agendamiento.";
    if(typeof ver_vetana_informativa == "function"){
        ver_vetana_informativa(mensaje, "", "warning");
    }else{
        alert(mensaje);
    }
}

function recalcularPlanMadreSeleccionadoAgenda(){
    agendaPlanMadreSeleccionadoId = "";
    for(var i = 0; i < agendaTratamientosSeleccionados.length; i++){
        var detalle = String(agendaTratamientosSeleccionados[i] || "");
        if(detalle !== "" && agendaTratamientosPlanMap[detalle]){
            agendaPlanMadreSeleccionadoId = String(agendaTratamientosPlanMap[detalle]);
            if(agendaPlanMadreAcordeonAbiertoId === ""){
                agendaPlanMadreAcordeonAbiertoId = agendaPlanMadreSeleccionadoId;
            }
            return;
        }
    }
...
function togglePlanMadreBloqueAgenda(planId){
    planId = String(planId || "");
    if(planId === ""){ return; }
    agendaPlanMadreAcordeonAbiertoId = agendaPlanMadreAcordeonAbiertoId === planId ? "" : planId;
    actualizarEstadoSeleccionPlanMadreAgenda();
}

function actualizarEstadoSeleccionPlanMadreAgenda(){
    var bloques = document.getElementsByClassName('agenda-plan-madre-bloque');
    for(var i = 0; i < bloques.length; i++){
        var planId = bloques[i].getAttribute('data-plan-id') || "";
        var abierto = planId !== "" && planId === agendaPlanMadreAcordeonAbiertoId;
        var seleccionado = agendaPlanMadreSeleccionadoId !== "" && planId !== "" && planId === agendaPlanMadreSeleccionadoId;
        if(abierto){
            bloques[i].classList.add('agenda-plan-madre-bloque--abierto');
        }else{
            bloques[i].classList.remove('agenda-plan-madre-bloque--abierto');
        }
        if(seleccionado){
            bloques[i].classList.add('agenda-plan-madre-bloque--seleccionado');
        }else{
            bloques[i].classList.remove('agenda-plan-madre-bloque--seleccionado');
        }
        if(agendaPlanMadreSeleccionadoId !== "" && planId !== "" && planId !== agendaPlanMadreSeleccionadoId){
            bloques[i].classList.add('agenda-plan-madre-bloque--bloqueado');
        }else{
            bloques[i].classList.remove('agenda-plan-madre-bloque--bloqueado');
        }
        var header = bloques[i].getElementsByClassName('agenda-plan-madre-bloque__header')[0];
        if(header){
            header.setAttribute('aria-expanded', abierto ? 'true' : 'false');
        }
        var chevron = bloques[i].getElementsByClassName('agenda-plan-madre-bloque__chevron')[0];
        if(chevron){
            chevron.innerHTML = abierto ? 'Ocultar' : 'Ver';
        }
    }
}
...
function toggleTratamientoAgenda(check){
    ...
    var planId = String(check.getAttribute('data-plan-id') || agendaTratamientosPlanMap[valor] || "");
    if(check.checked && agendaPlanMadreSeleccionadoId !== "" && planId !== "" && planId !== agendaPlanMadreSeleccionadoId){
        check.checked = false;
        mostrarAvisoPlanMadreDiferenteAgenda();
        return;
    }
    ...
    if(check.checked && idx < 0){
        ...
        if(planId !== ""){
            agendaPlanMadreSeleccionadoId = planId;
            agendaPlanMadreAcordeonAbiertoId = planId;
        }
    }else if(!check.checked && idx >= 0){
        agendaTratamientosSeleccionados.splice(idx, 1);
        recalcularPlanMadreSeleccionadoAgenda();
    }
    actualizarVentaSeleccionadaDesdeTratamientosAgenda();
    actualizarEstadoSeleccionPlanMadreAgenda();
    actualizarPrevisionNuevaCitaAgenda();
...
function verCerrarAsignarTratamiento(mostrar){
    ...
    if(mostrar){
        ...
        cargarPlanMadrePacienteAgenda(
            evento ? (evento.cod_cliente || evento.id_paciente || document.getElementById('detAgendaPacienteId').innerHTML) : document.getElementById('detAgendaPacienteId').innerHTML,
            'divTratamientosAgenda',
            idAbmAgenda,
            'detAgendaPlanMadreEstadoAsignar'
        );
    }else{
        if(overlay){
            overlay.style.display = 'none';
...
function limpiarFichaPacienteCalendario() {
    idAbmAgenda= "";
    agendaEventoDetalleActual = null;
    resetDeudaDetalleAgenda();
    document.getElementById('detAgendaDeudasPendientes').innerHTML = '';
...
function cambiarEstadoAgendaDesdeModal(nuevoEstado, opciones){
    if(agendaDetalleAccionEnProceso){
        return;
    }
    var evento = agendaEventoDetalleActual || {};
    var estadoActual = estadoPrincipalDetalleAgenda(evento.estado);
    if(esEstadoFinalDetalleAgenda(evento.estado) && String(nuevoEstado || "").toUpperCase() !== estadoActual){
        cerrarMasAccionesDetalleAgenda();
        return;
    }
    var idAgenda = document.getElementById('detAgendaId').innerHTML;
...
function actualizarHorarioAgendaDesdeModal(){
    var idAgenda = document.getElementById('detAgendaId').innerHTML;
    var fecha = document.getElementById('detAgendaFechaReprogramar') ? document.getElementById('detAgendaFechaReprogramar').value : (agendaEventoDetalleActual ? agendaEventoDetalleActual.fecha : '');
    ...
    if(fecha == ''){
        alert('Debe cargar la fecha de la nueva cita.');
        return;
    }

    if(typeof esDomingoFechaAgenda === "function" && esDomingoFechaAgenda(fecha)){
    ...
    if(agendaEventoDetalleActual
        && String(fecha) === String(agendaEventoDetalleActual.fecha || '')
        && String(horaInicio) === String(agendaEventoDetalleActual.inicio || '')
        && String(horaFin) === String(agendaEventoDetalleActual.fin || '')
    ){
        alert('Debe seleccionar una fecha u horario distinto para reprogramar.');
        return;
    }

    reprogramarAgendaConCopia(idAgenda, fecha, horaInicio, horaFin);
}

function reprogramarAgendaConCopia(idAgenda, fecha, horaInicio, horaFin){
    obtener_datos_user();
	...
	verCerrarEfectoCargando("1");
    setAccionesDetalleAgendaProcesando(true);
    $.ajax({
        data: datos,
        url: "/GoodVentaAsisCap/php_system/abmCalendar.php",
        type: "post",
        ...
        success: function (responseText) {
            ...
            try {
                ...
                if (Respuesta == true) {
                    cerrarDetalleAgenda();
                    if(document.getElementById('inptFechaAgenda')){
                        document.getElementById('inptFechaAgenda').value = fecha;
                    }
                    if(typeof actualizarTextoFechaAgenda === "function"){
                        actualizarTextoFechaAgenda();
                    }
                    cargarAgendaConsultoriosDesdePHP();
                    if(ventanaAnterior && ventanaAnterior.length > 0){
                        ventanaAnterior.pop();
                    }
                    limpiarFichaPacienteCalendario();
                    if(typeof ver_vetana_informativa === "function"){
                        ver_vetana_informativa(resp["mensaje"] || "Cita reprogramada correctamente");
                    }
                } else {
                    alert(resp["mensaje"] || "No se pudo reprogramar la cita.");
                    setAccionesDetalleAgendaProcesando(false);
                }
            } catch (error) {
                var titulo = "Error reprogramar agenda: " + error + " \r\n Consola: " + responseText;
                GuardarArchivosLog(titulo);
                console.log(responseText);
                setAccionesDetalleAgendaProcesando(false);
            }
...
function limpiarFormularioNuevaCita(){
    ...
    document.getElementById('inptMotivoAgenda').value = '';
    document.getElementById('inptDoctorNuevaCita').innerHTML = '';
    if(document.getElementById('btnGuardarCitaAgenda')){
        document.getElementById('btnGuardarCitaAgenda').disabled = false;
    }
    if(document.getElementById('inptBuscarPacienteAgendaModal')){
        document.getElementById('inptBuscarPacienteAgendaModal').value = '';
    }
    if(document.getElementById('divTablaPacientesAgenda')){
...
function seleccionarPacienteAgenda(idPaciente, nombrePaciente){
    ...
    cargarPlanMadrePacienteAgenda(idPaciente, 'divTratamientosNuevaCitaAgenda', '', 'divPlanMadreNuevaCitaAgenda');
    ...
    $.ajax({
        ...
        success: function (responseText) {
            try {
                ...
                if (resp["1"] == "exito") {
                    if (resp["3"] == 1) {
                        if(controlVentana === 'detalle_agendamiento'){
                            renderHistorialDetalleAgenda(tabla, resp["2"]);
                        }else{
