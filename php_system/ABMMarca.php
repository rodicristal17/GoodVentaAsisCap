...
if($funt=="nuevo" || $funt=="editar")
{
	...
	abm($cod_marcas,$descripcion,$Estado,$funt);
...
function abm($cod_marcas,$descripcion,$Estado,$funt)
{
...
exit;
	...
	if($funt=="nuevo")
	{
				$consulta= "Select count(*) from marcas where descripcion=? and Estado ='Activo' ";
...
if($valor==1)
{
    ...
    $consulta="insert into marcas (descripcion,Estado) values (?,?)";	
     $stmt = $mysqli->prepare($consulta);
    $ss='ss';
    ...
    $consulta="Update marcas set descripcion=?,Estado=? where cod_marcas=?";	
    ...
    $stmt->bind_param($ss,$descripcion,$Estado,$cod_marcas); 
...
function buscar($buscar,$Estado)
{
	$mysqli=conectar_al_servidor();
	 $pagina='';
        ...
        from marcas where descripcion like ?  and Estado=? order by descripcion asc ";
...
if ( ! $stmt->execute()) {
 ...
 if ($valor>0)
 {
	  while ($valor= mysqli_fetch_assoc($result))
	  {
			  ...
			  <tr id='tbSelecRegistro' onclick='ObtenerdatosAbmMarca(this)'>
			  <td id='td_id' style='display:none;'>".$cod_marcas."</td>
