...
function abmPresupuesto(cod_presupuesto, cant_cuotas, cod_clienteFK, cod_ventaFK, plan_vendido, opciones) {
	...
	var datos = {
		...
		"confirmar_identidad_policial": opciones.confirmarIdentidadPolicial === true ? "1" : "0",
	};
	$.ajax({
		data: datos,
			...
			try {
				...
				if (Respuesta === "IDENTIDAD_DIFERENTE") {
					abrirModalIdentidadPolicial(datos, function () {
						var opcionesReintento = $.extend({}, opciones, {
							confirmarIdentidadPolicial: true,
							conservarDatosEnError: true
						});
						abmPresupuesto(cod_presupuesto, cant_cuotas, cod_clienteFK, cod_ventaFK, plan_vendido, opcionesReintento);
					});
					return;
				}
